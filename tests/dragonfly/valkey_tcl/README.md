...
Runs as the `valkey-tcl-stream` job in `.github/workflows/ci.yml`. It downloads the
`dragonfly-binary` artifact built by the `build` job (no rebuild), installs `tcl` +
runtime deps, syncs the harness, and runs the suites.
